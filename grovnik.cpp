...
grovnik::grovnik(char character) { this->character = character; }
...
char grovnik::get_character() const { return character; }
...
// Friend function to allow reading grovnik and derived grovnik classes from
// streams
...
int binocular::get_cost() { return cost; }

// returns the binoculars vision range
int binocular::get_range() { return range; }
...
void binocular::read(istream &source) {
  ...
  if (temp.find_first_not_of(" \n\r\t") != string::npos)
    throw("Incorrect binoculars: found unexpected text on line after "
          "<distance>.");
}
...
// constructor with args
ship::ship(int cost) : grovnik('S') { this->cost = cost; }
...
treasure_chest::treasure_chest()
    : grovnik('$'), name(NULL), amount(0), discovered(false) {}

treasure_chest::treasure_chest(char *name, int amount)
    : grovnik('$'), discovered(false) {
...
char *treasure_chest::get_name() { return name; }

int treasure_chest::get_amount() { return amount; }
...
void royal_diamond::read(istream &source) {
  ...
  if (temp.find_first_not_of(" \n\r\t") != string::npos)
    throw("Incorrect diamonds: found unexpected text after on line after <y "
          "location>.");
}
...
// default constructor
obstacle::obstacle()
    : toolObstacle('!'), description(NULL), kind(0), b_energy(0) {}

// constructor with args
obstacle::obstacle(char *description, int kind, int b_energy)
    : toolObstacle('!') {
...
const char *obstacle::get_description() { return description; }

int obstacle::get_b_energy() { return (-b_energy); }

const char *obstacle::get_kind_text() { return obstacleTypes[kind].c_str(); }

int obstacle::get_kind_int() { return kind; }
...
void obstacle::read(istream &source) {
  ...
  if (temp.length() == 0)
    throw "Incomplete obstacle: missing <kind>, <energy cost> and "
          "<description>.";
...
tool::tool()
    : toolObstacle('T'), description(NULL), target_count(0), kind(0), cost(0),
      divisor(1) {}

// constructor with args
tool::tool(char *description, int kind, int cost, int divisor)
    : toolObstacle('T') {
  this->description = new char[strlen(description) + 1];
  strcpy(this->description, description);
...
bool tool::check_equal(const char *item) {
  ...
  if (this->description == NULL)
    return false;
  if (strcmp(this->description, item) == 0)
    return true;
  else
    return false;
}
...
const char *tool::get_description() { return description; }
...
const char *tool::get_kind_text() { return toolTypes[kind].c_str(); }
...
void tool::read(istream &source) {
  ...
  if (temp.length() == 0)
    throw "Incomplete tool: missing <kind>, <target_count>, <targets...>, "
          "<divisor>, <cost> and <description>.";
  ...
  for (int i = 0; i < target_count; ++i) {
    source >> temp;
    if (temp.length() == 0)
      throw "Incomplete tool: missing (or too few) <targets...>, <divisor>, "
...
char *food::get_name() { return name; }

int food::get_cost() { return cost; }

int food::get_energy() { return energy; }
...
char *clue::get_clue() { return clueText; }

void clue::discover() { discovered = true; }
