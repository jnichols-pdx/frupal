...
int Hero::visionRange() { return vision; }
...
bool Hero::selectTool(
  ...
  for (int i = 0; i < items; ++i) {
    if (inventory[i]->check_if_targets(obstacleType) == true)
      ++counter; // check if there is no items in our inventory that can break
                 // the obstacle
  }
  if (counter == 0)
    return false; // no items, return
...
int Hero::select(int obstacleType, int menuRow) {
  ...
  do {
    ...
    inventory[arrPos]->display_name(
        startRow - 2); // display one at a time for simplicity sake
    ...
    refresh();
  } while ((inventory[arrPos]->check_if_targets(obstacleType) == false) ||
           userInput !=
               char(10)); // continue to select until user makes valid decision
  return arrPos;
}

// asks the user if they want to buy an item, checks if its food or tools or
// binoculars
bool Hero::purchaseItem(grovnik *item) {
  ...
  if (userInput == 'y' || userInput == 'Y') {
    ...
    if (binocPtr) { // its binoculars, improve hero's sight
      ...
      if (vision < range) // don't downgrade if the user purchases another,
                          // worse, pair of binoculars
        vision = range;
      binocPtr = NULL;
      return true;
    }
